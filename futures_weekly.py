...
"""NIFTY Futures Weekly Heikin-Ashi Reversal Scanner.

Scans the 210 NSE F&O universe for weekly Heikin-Ashi reversals with
position sizing based on contract lot sizes and rupee risk.
"""

SIGNAL_DATE = "2026-08-28"
BASE_DIR = Path(__file__).resolve().parent
CONSTITUENTS_FILE = BASE_DIR / "ind_nifty_futures_list.csv"
BATCH_SIZE = 50
YF_PERIOD = "max"
...
def load_symbols():
    ...
    if "Market Lot" not in df.columns:
        df["Market Lot"] = np.nan
    return df
...
def weekly_ohlc(daily):
    ...
    return compute_heikin_ashi(weekly)


def count_prior_streak(frame, pos, streak_type="bearish"):
    """Counts consecutive prior HA candles of the specified type."""
    count = 0
    j = pos - 1
    flag_col = "HA_Bearish" if streak_type == "bearish" else "HA_Bullish"

    while j >= 0:
        prev_period = frame.index[j].to_period("W-FRI")
        next_period = frame.index[j + 1].to_period("W-FRI")
...
def find_weekly_signals(weekly, signal_date, market_lot=np.nan):
    """Detects both Bullish and Bearish Heikin-Ashi reversals for the given week."""
    if weekly.empty or len(weekly) < 4:
        return None, None

    weekly = weekly.copy()
    target_dt = pd.Timestamp(signal_date)
    target_period = target_dt.to_period("W-FRI")
    weekly["WP"] = weekly.index.to_period("W-FRI")

    rows = weekly[weekly["WP"] == target_period]
    if rows.empty:
        return None, None

    signal_idx = rows.index[-1]
    signal = weekly.loc[signal_idx]
    pos = weekly.index.get_loc(signal_idx)
    week_start = signal_idx - pd.Timedelta(days=4)

    bullish_sig = None
    bearish_sig = None

    lot_val = float(market_lot) if pd.notna(market_lot) else np.nan

    # 1. Bullish Reversal Check (Prior 3+ Red HA -> Current 1 Green HA)
    if bool(signal["HA_Bullish"]):
        bearish_count, j = count_prior_streak(weekly, pos, streak_type="bearish")
        if bearish_count >= 3:
            first_bearish = weekly.index[j + 1]
            ...
            risk_per_lot = round(risk * lot_val, 2) if pd.notna(lot_val) and risk > 0 else np.nan

            bullish_sig = {
                "Signal Type": "Bullish Reversal (Long)",
                "Streak Count": bearish_count,
                "Streak Type": "Bearish Weeks",
                "First Streak Date": first_bearish.strftime("%Y-%m-%d"),
                "Reversal Week Start": week_start.strftime("%Y-%m-%d"),
                "Reversal Week End": signal_idx.strftime("%Y-%m-%d"),
                "Market Lot": lot_val,
                "Entry": entry,
                "SL": sl,
                "Risk": risk,
                "Risk/Lot (INR)": risk_per_lot,
                "Target 1": t1,
                "Target 2": t2,
    ...
    if bool(signal["HA_Bearish"]):
        bullish_count, j = count_prior_streak(weekly, pos, streak_type="bullish")
        if bullish_count >= 3:
            first_bullish = weekly.index[j + 1]
            ...
            bearish_sig = {
                ...
                "Streak Type": "Bullish Weeks",
                "First Streak Date": first_bullish.strftime("%Y-%m-%d"),
                "Reversal Week Start": week_start.strftime("%Y-%m-%d"),
                "Reversal Week End": signal_idx.strftime("%Y-%m-%d"),
                "Market Lot": lot_val,
                "Entry": entry,
...
def score_and_rank_signals(df, is_bullish=True):
    ...
    if is_bullish:
        ...
        # Close Strength % = (Close - Low) / (High - Low) * 100
...
def main():
    parser = argparse.ArgumentParser(description="NIFTY Futures Weekly Heikin-Ashi Reversal Scanner")
    parser.add_argument("--date", default=SIGNAL_DATE, help="YYYY-MM-DD date within target week (e.g. 2026-08-28)")
    args = parser.parse_args()
    target_date = args.date

    target_dt = pd.Timestamp(target_date)
    week_friday = (target_dt + pd.offsets.Week(weekday=4)).strftime("%Y-%m-%d") if target_dt.weekday() != 4 else target_date
    ...
    print("==========================================================")
    print("  NIFTY FUTURES (F&O) WEEKLY HEIKIN-ASHI REVERSAL SCANNER")
    print(f"  Target Week: {target_date} (Week Ending {week_friday})")
    print(f"  Universe: {len(universe)} Futures Stocks")
    print("==========================================================")
    ...
    for start in range(0, len(tickers), BATCH_SIZE):
        ...
        for yahoo_symbol in batch:
            ...
            market_lot = row.get("Market Lot", np.nan)
            ...
            try:
                ...
                w_ohlc = weekly_ohlc(stock_daily)
                b_sig, s_sig = find_weekly_signals(w_ohlc, target_date, market_lot=market_lot)
                if b_sig:
    ...
    out_stem = f"NIFTY_Futures_Weekly_{week_friday}_HA_Reversal_Scan"
    ...
    cols_order = [
        ...
        "Move %", "Close Pos %", "Risk %", "Market Lot", "Risk/Lot (INR)",
        "Entry", "SL", "Risk", "Target 1", "Target 2",
        "Streak Type", "First Streak Date", "Reversal Week Start", "Reversal Week End",
    ...
    with pd.ExcelWriter(out_xlsx, engine="openpyxl") as writer:
        ...
        universe.to_excel(writer, sheet_name="NIFTY Futures Universe", index=False)
        if failed:
            pd.DataFrame({"YahooSymbol": sorted(set(failed))}).to_excel(
                writer, sheet_name="Download Failures", index=False
            )

    print(f"\n================ NIFTY FUTURES HEIKIN-ASHI SCAN (WEEK ENDING {week_friday}) ================")
    ...
    if not bullish_df.empty:
        ...
        print(bullish_df[[
            ...
            "Market Lot", "Entry", "SL", "Risk/Lot (INR)", "Target 1", "Target 2"
    ...
    if bullish_df.empty and bearish_df.empty:
        print(f"\nNo qualifying Heikin-Ashi reversals found for week ending {week_friday}.")
