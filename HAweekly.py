...
"""NIFTY 50 Weekly Heikin-Ashi Reversal Scanner.

Scans for:
  - Bullish Reversals: >=3 consecutive red HA candles followed by 1 green HA candle
  - Bearish Reversals: >=3 consecutive green HA candles followed by 1 red HA candle
"""

SIGNAL_DATE = "2026-08-28"
BASE_DIR = Path(__file__).resolve().parent
CONSTITUENTS_FILE = BASE_DIR / "ind_nifty50list.csv"
BATCH_SIZE = 50
YF_PERIOD = "max"
...
def compute_heikin_ashi(df):
    ...
    # Keep original OHLC for actual trade level executions
    ha["Open"] = df["Open"]
    ha["High"] = df["High"]
    ha["Low"] = df["Low"]
    ha["Close"] = df["Close"]
...
def count_prior_streak(frame, pos, streak_type="bearish"):
    """Counts consecutive prior HA candles of the specified type (bearish or bullish)."""
    count = 0
    j = pos - 1
...
def find_weekly_signals(weekly, signal_date):
...
def main():
    parser = argparse.ArgumentParser(description="NIFTY 50 Weekly Heikin-Ashi Reversal Scanner")
    ...
    print("==========================================================")
    print("  NIFTY 50 WEEKLY HEIKIN-ASHI REVERSAL SCANNER")
    print(f"  Target Week: {target_date} (Week Ending {week_friday})")
    print(f"  Universe: {len(universe)} Nifty 50 stocks")
    print("==========================================================")
    print("Downloading historical cash-market data...")
    ...
    for start in range(0, len(tickers), BATCH_SIZE):
        ...
        for yahoo_symbol in batch:
            ...
            try:
                ...
                b_sig, s_sig = find_weekly_signals(w_ohlc, target_date)
                if b_sig:
    ...
    out_stem = f"NIFTY50_Weekly_{week_friday}_HA_Reversal_Scan"
    ...
    cols_order = [
        ...
        "Move %", "Close Pos %", "Risk %", "Streak Type", "First Streak Date",
        "Reversal Week Start", "Reversal Week End",
    ...
    with pd.ExcelWriter(out_xlsx, engine="openpyxl") as writer:
        ...
        universe.to_excel(writer, sheet_name="NIFTY 50 Universe", index=False)
        if failed:
            pd.DataFrame({"YahooSymbol": sorted(set(failed))}).to_excel(
                writer, sheet_name="Download Failures", index=False
            )

    print(f"\n================ NIFTY 50 HEIKIN-ASHI SCAN (WEEK ENDING {week_friday}) ================")
