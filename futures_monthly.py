...
"""NIFTY Futures Monthly Heikin-Ashi Reversal Scanner.

Scans the 210 NSE F&O universe for monthly Heikin-Ashi reversals with
...
def find_monthly_signals(monthly, signal_month, market_lot=np.nan):
...
def main():
    parser = argparse.ArgumentParser(description="NIFTY Futures Monthly Heikin-Ashi Reversal Scanner")
    ...
    print("==========================================================")
    print("  NIFTY FUTURES (F&O) MONTHLY HEIKIN-ASHI REVERSAL SCANNER")
    ...
    for start in range(0, len(tickers), BATCH_SIZE):
        ...
        for yahoo_symbol in batch:
            ...
            try:
                ...
                b_sig, s_sig = find_monthly_signals(m_ohlc, target_month, market_lot=market_lot)
                if b_sig:
    ...
    out_stem = f"NIFTY_Futures_Monthly_{target_month}_HA_Reversal_Scan"
    ...
    cols_order = [
        ...
        "Streak Type", "First Streak Month", "Reversal Month", "Reversal Date",
    ...
    print(f"\n================ NIFTY FUTURES HEIKIN-ASHI SCAN ({label.upper()}) ================")
